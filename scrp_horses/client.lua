        ...
        if IsPlayerTargettingAnything(id) then
            ...
            if PromptHasStandardModeCompleted(BrushPrompt[entity]) then -- 0x4CC0E2FE
                ...
                if distance < 1.75 then
                    TriggerServerEvent('scrp_horses:requestbrush', entity)
                end
            end
            ...
            if PromptHasStandardModeCompleted(FeedPrompt[entity]) then -- 0xE30CD707
                ...
                local distance = #(coords - coordshorse)
                if distance < 1.75 then
                    TriggerServerEvent('scrp_horses:requestfeed', entity)
                end
            end
        end
    end
...
function Feed(player, horse, increase)
    TriggerServerEvent("scrp_horses:Consume")
...
RegisterNetEvent('scrp_horses:Brush')
AddEventHandler('scrp_horses:Brush', function (horse)
    local player = PlayerPedId()
    if horse ~= nil then       
        local coords = GetEntityCoords(player)
...
RegisterNetEvent('scrp_horses:Feed')
AddEventHandler('scrp_horses:Feed', function(horse, increase)
    local player = PlayerPedId()
    if horse == false then
        if IsPedOnMount(player) then
            local horse = GetMount(player)
